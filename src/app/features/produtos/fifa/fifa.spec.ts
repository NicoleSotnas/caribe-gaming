...
const mockRawgService = {
  obterDetalhesJogo: vi.fn().mockReturnValue(of({
    ...
    released: '2023-09-29',
    developers: [{ name: 'EA Canada / EA Romania' }],
    publishers: [{ name: 'Electronic Arts' }],
    platforms: [{ platform: { name: 'PC' } }],
    background_image: 'fc24.jpg'
  })),
...
describe('Componente Fifa - Testes de Cobertura Alta', () => {
  ...
  describe('Lógica de Métodos e Estado (Vitest)', () => {
    ...
    beforeEach(() => {
      ...
      const fixture = TestBed.createComponent(Fifa);
    ...
    it('deve inicializar o componente e carregar dados do jogo', () => {
      ...
      expect(component.jogo?.nome).toBe('EA SPORTS FC 24');
    ...
    it('deve manipular o modal de avaliação e validação de envio', () => {
      ...
      component.novoNome = 'ProGamer';
      component.novoTexto = 'Excelente jogabilidade e animações!';
      component.enviarAvaliacao();
      expect(component.listaComentarios[0].autor).toBe('ProGamer');
    ...
    it('deve formatar estrelas de avaliação corretamente', () => {
      const estrelas = component.getEstrelasTexto(4);
      expect(estrelas).toBe('★★★★☆');
    });

    it('deve adicionar ao carrinho, favoritar e acionar comprar agora', () => {
      component.jogo = { nome: 'EA SPORTS FC 24', plataformas: 'PC' } as any;
  ...
  describe('Interface do Usuário (Testing Library)', () => {
    ...
    it('deve renderizar a página do jogo e checar comentários na tela', async () => {
      ...
      // Valida se o comentário padrão de ProGamer_FC aparece visível na página
      const autorComentario = screen.getByText(/ProGamer_FC/i);

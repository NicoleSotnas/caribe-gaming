...
const CHAVE_CACHE = 'hollowknight_dados_completos';
const CHAVE_AVALIACOES = 'hollowknight_avaliacoes_lista';

@Component({
  selector: 'app-hollow-knight',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './hollow-knight.html',
  styleUrl: './hollow-knight.css',
})
export class HollowKnight implements OnInit {
  ...
  listaComentarios: Comentario[] = [
    {
      ...
      avatar: 'E',
      estrelas: 5,
      texto: 'Uma das maiores obras-primas indie de todos os tempos. Direção de arte incrível, trilha sonora inesquecível e combate impecável.',
      data: '21/08/2026',
      likes: 380,
      dislikes: 2,
    },
    {
      id: 2,
      autor: 'HornetProtector',
      avatar: 'H',
      estrelas: 5,
      texto: 'Exploração gratificante e chefes memoráveis. O nível de detalhes do mundo subterrâneo é absurdo!',
      data: '19/08/2026',
      likes: 215,
      dislikes: 1,
    },
  ...
  readonly ID_PRODUTO = '9';
  readonly precoJogo = 46.99;
  readonly precoFormatado = 'R$ 46,99';
  ...
  private jogoSlug = 'hollow-knight';
  private steamAppId = '367520';
  ...
  adicionarAoCarrinho(): void {
    ...
    this.carrinhoFacade.adicionarProduto({
      ...
      plataforma: this.jogo.plataformas || 'PC / PS4 / Switch',
      categoria: 'Metroidvania, Ação',
    });
  ...
  toggleFavorito(): void {
    ...
    this.favoritosService.toggleFavorito({
      ...
      genero: 'Metroidvania',
      plataforma: this.jogo.plataformas || 'PC / PS4 / Switch',
      categorias: ['metroidvania', 'acao', 'indie'],
    });
    ...
    this.rawgService.obterDetalhesJogo(this.jogoSlug).subscribe({
      next: (res: any) => {
        this.jogo = {
          id: res.id,
          nome: res.name,
          descricao: `Forje seu próprio caminho em Hollow Knight! Uma aventura épica em estilo metroidvania 2D através de um vasto reino arruinado de insetos e heróis. Explore cavernas sinuosas, lute contra criaturas contaminadas e faça amizade com insetos bizarros, tudo em um estilo clássico desenhado à mão. Desvende os mistérios do antigo reino de Hallownest enquanto enfrenta desafios de plataforma e combate extremamente precisos.`,
          dataLancamento: res.released ? res.released.split('-').reverse().join('/') : '24/02/2017',
          desenvolvedoras: res.developers?.[0]?.name || 'Team Cherry',
          distribuidoras: res.publishers?.[0]?.name || 'Team Cherry',
          classificacaoEtaria: '10+',
          plataformas: res.platforms?.map((p: any) => p.platform.name).join(' / ') || 'PC / PlayStation / Xbox / Switch',

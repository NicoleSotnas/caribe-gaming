...
const CHAVE_CACHE = 'marvelrivals_dados_completos';
const CHAVE_AVALIACOES = 'marvelrivals_avaliacoes_lista';

@Component({
  selector: 'app-marvel-rivals',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './marvel-rivals.html',
  styleUrl: './marvel-rivals.css',
})
export class MarvelRivals implements OnInit {
  ...
  listaComentarios: Comentario[] = [
    {
      ...
      estrelas: 5,
      texto:
        'A jogabilidade é super rápida e fluida! Os combos combinados entre heróis trazem uma mecânica muito divertida para o gênero.',
      data: '18/05/2026',
      likes: 245,
      dislikes: 3,
    },
    {
      id: 2,
      autor: 'DoctorStrange_MCU',
      avatar: 'D',
      estrelas: 5,
      texto: 'O visual estilizado em quadrinhos ficou perfeito e a destruição dos cenários deixa cada partida imprevisível.',
      data: '15/05/2026',
      likes: 160,
      dislikes: 2,
    },
  ...
  readonly ID_PRODUTO = '19';
  readonly precoJogo = 0;
  readonly precoFormatado = 'Grátis';
  ...
  private jogoSlug = 'marvel-rivals';
  private steamAppId = '2767030';
  ...
  adicionarAoCarrinho(): void {
    ...
    this.carrinhoFacade.adicionarProduto({
      ...
      categoria: 'Hero Shooter, Ação, PvP',
    });

    this.exibirToast('🎁 Jogo resgatado na sua conta!');
  }
  ...
  private carregarDadosDoJogo(): void {
    this.rawgService.obterDetalhesJogo(this.jogoSlug).subscribe({
      next: (res: any) => {
        this.jogo = {
          id: res.id,
          nome: res.name || 'Marvel Rivals',
          descricao: `Marvel Rivals é um jogo de tiro em equipe de Super-Heróis em terceira pessoa. Monte um esquadrão estelar da Marvel, elabore estratégias combinando poderes para formar habilidades únicas de Sinergia de Equipe e lute em campos de batalha mutáveis e destrutíveis em todo o Multiverso Marvel em constante evolução.`,
          dataLancamento: res.released ? res.released.split('-').reverse().join('/') : '06/12/2024',
          desenvolvedoras: res.developers?.[0]?.name || 'NetEase Games',
          distribuidoras: res.publishers?.[0]?.name || 'NetEase Games',

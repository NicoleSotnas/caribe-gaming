...
const CHAVE_CACHE = 'lifeisstrange_dados_completos';
const CHAVE_AVALIACOES = 'lifeisstrange_avaliacoes_lista';

@Component({
  selector: 'app-life-is-strange',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './life-is-strange.html',
  styleUrl: './life-is-strange.css',
})
export class LifeIsStrange implements OnInit {
  ...
  listaComentarios: Comentario[] = [
    {
      id: 1,
      autor: 'ChloePrice_Punk',
      avatar: 'C',
      estrelas: 5,
      texto: 'Uma montanha-russa de emoções. A história, as escolhas e a trilha sonora marcam demais. Impossível terminar sem chorar.',
      data: '22/08/2026',
      likes: 310,
      dislikes: 2,
    },
    {
      ...
      avatar: 'P',
      estrelas: 5,
      texto: 'A mecânica de voltar no tempo e ver as consequências das escolhas é incrível. Um dos melhores jogos narrativos que já joguei.',
      data: '19/08/2026',
      likes: 180,
      dislikes: 1,
    },
  ...
  readonly ID_PRODUTO = '14';
  readonly precoJogo = 19.80;
  readonly precoOriginalFormatado = 'R$ 99,00';
  readonly precoFormatado = 'R$ 19,80';
  ...
  private jogoSlug = 'life-is-strange-episode-1-2';
  private steamAppId = '319630';
  ...
  toggleFavorito(): void {
    ...
    this.favoritosService.toggleFavorito({
      ...
      desconto: '-80%',
      genero: 'Aventura / Escolhas',
      plataforma: this.jogo.plataformas || 'PC / PS4 / Xbox',
      categorias: ['aventura', 'escolhas', 'drama'],
    });
    ...
    this.rawgService.obterDetalhesJogo(this.jogoSlug).subscribe({
      next: (res: any) => {
        this.jogo = {
          ...
          descricao: `Life is Strange é uma premiada aventura gráfica episodicamente contada que permite ao jogador voltar no tempo e alterar o passado, presente e futuro. Acompanhe a história de Max Caulfield, uma estudante de fotografia que descobre a capacidade de voltar no tempo ao salvar sua melhor amiga, Chloe Price. A dupla logo se vê investigando o desaparecimento misterioso da estudante Rachel Amber, desvendando um lado sombrio da vida na pacata cidade de Arcadia Bay.`,
          dataLancamento: res.released ? res.released.split('-').reverse().join('/') : '29/01/2015',
          desenvolvedoras: res.developers?.[0]?.name || 'DONTNOD Entertainment',
          distribuidoras: res.publishers?.[0]?.name || 'Square Enix',

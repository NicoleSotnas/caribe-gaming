...
describe('Componente GrandTheftAutoV - Testes de Cobertura Alta', () => {
  ...
  describe('Lógica de Métodos e Estado (Vitest)', () => {
    ...
    beforeEach(() => {
      ...
      const fixture = TestBed.createComponent(GrandTheftAutoV);
    ...
    it('deve inicializar o componente e carregar dados do jogo', () => {
      ...
      expect(component.jogo?.nome).toBe('Grand Theft Auto V');
      expect(component.galeriaImagens.length).toBeGreaterThan(0);
    });

    it('deve calcular corretamente as propriedades dinâmicas de avaliação', () => {
      expect(component.totalVotosGeral).toBeGreaterThan(34200);
      expect(component.mediaNotasDinamica).toBe('4.8');

      const porcentagemCincoEstrelas = component.getPorcentagemEstrelas(5);
      expect(porcentagemCincoEstrelas).toBeGreaterThanOrEqual(2);
      expect(porcentagemCincoEstrelas).toBeLessThanOrEqual(95);
    });
    ...
    it('deve manipular o modal de avaliação e validação de envio', () => {
      ...
      component.novoNome = 'Trevor';
      component.novoTexto = 'Jogo incrivel e insano!';
      component.enviarAvaliacao();
      expect(component.listaComentarios[0].autor).toBe('Trevor');
    ...
    it('deve adicionar ao carrinho, favoritar e acionar comprar agora', () => {
      component.jogo = { nome: 'Grand Theft Auto V', plataformas: 'PC' } as any;
  ...
  describe('Interface do Usuário (Testing Library)', () => {
    ...
    it('deve renderizar a página do jogo e checar comentários na tela', async () => {
      ...
      // Valida se o comentário padrão do TrevorPhilips aparece visível na página
      const autorComentario = screen.getByText(/TrevorPhilips/i);

...
import { MarvelRivals } from '../marvel-rivals/marvel-rivals';
...
const mockRawgService = {
  obterDetalhesJogo: vi.fn().mockReturnValue(of({
    ...
    name: 'Marvel Rivals',
    released: '2024-12-06',
    developers: [{ name: 'NetEase Games' }],
    publishers: [{ name: 'NetEase Games' }],
    platforms: [{ platform: { name: 'PC' } }],
    background_image: 'marvelrivals.jpg'
  })),
...
describe('Componente MarvelRivals - Testes de Cobertura Alta', () => {
  ...
  describe('Lógica de Métodos e Estado (Vitest)', () => {
    ...
    beforeEach(() => {
      ...
      const fixture = TestBed.createComponent(MarvelRivals);
    ...
    it('deve inicializar o componente e carregar dados do jogo', () => {
      ...
      expect(component.jogo?.nome).toBe('Marvel Rivals');
    ...
    it('deve manipular o modal de avaliação e validação de envio', () => {
      ...
      component.novoNome = 'Spidey';
      component.novoTexto = 'Combos de heróis sensacionais e partida muito rápida!';
      component.enviarAvaliacao();
      expect(component.listaComentarios[0].autor).toBe('Spidey');
    ...
    it('deve resgatar o jogo gratuito no carrinho, favoritar e acionar comprar agora', () => {
      component.jogo = { nome: 'Marvel Rivals', plataformas: 'PC' } as any;

      component.adicionarAoCarrinho();
      expect(mockCarrinhoFacade.adicionarProduto).toHaveBeenCalled();
      expect(component.mensagemToast).toContain('resgatado na sua conta');
  ...
  describe('Interface do Usuário (Testing Library)', () => {
    ...
    it('deve renderizar a página do jogo e checar comentários na tela', async () => {
      ...
      // Valida se o comentário padrão do SpiderFan_BR aparece visível na página
      const autorComentario = screen.getByText(/SpiderFan_BR/i);
